...
plt.title('4 - Washington Transit Frequency')
map = Basemap(resolution="h", projection="stere", width=80000, height=80000, lon_0=-77.0363700
, lat_0=38.8951100)
...
# WMATA data is missing calendar.txt.
# plotData(map, ['WMATA Data'], min_draw_size)

# Having issues reading data from ART, because python doesn't like comma separated files with quotes in them.
# plotData(map, ['ART Data'], min_draw_size)

plotData(map, ['PRTC Data'], min_draw_size)

plotData(map, ['Ride On Data'], min_draw_size)

plotData(map, ['Fairfax Data'], min_draw_size)

plotData(map, ['DASH Data'], min_draw_size)

plotData(map, ['CMRT Data'], min_draw_size)

plotData(map, ['MTA Data'], min_draw_size)

plt.savefig('map4.5.png')
plt.show()