...
plt.title('2 - Los Angeles Transit Frequency')
map = Basemap(resolution="h", projection="stere", width=80000, height=80000, lon_0=-118.243683, lat_0=34.052235)

plotData(map, ['OCTA Data'], min_draw_size)

# PVPTA data does not have shapes.txt.
# plotData(map, ['PVPTA Bus Data'], min_draw_size)

# Torrance shapes.txt does not exist.
# plotData(map, ['Torrance Data'], min_draw_size)

plotData(map, ['Big Blue Bus Data'], min_draw_size)

plotData(map, ['Foothill Transit Data'], min_draw_size)

# Glendale data lacks shapes.txt.
# plotData(map, ['Glendale Data'], min_draw_size)

plotData(map, ['LADOT Data'], min_draw_size)

plotData(map, ['LA Metro Bus Data'], min_draw_size)

plotData(map, ['LA Metro Rail Data'], min_draw_size * 2)

la_rail_stops = pd.read_csv('../LA Metro Rail Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
for index, row in la_rail_stops.iterrows():
...
plotData(map, ['Metrolink Data'], min_draw_size * 2)

metrolink_stops = pd.read_csv('../Metrolink Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
for index, row in metrolink_stops.iterrows():
    map.plot(row['stop_lon'], row['stop_lat'], marker='o', markersize=min_draw_size, markeredgecolor='white', markerfacecolor='black', markeredgewidth=min_draw_size * .5, latlon=True)
