...
plt.title('5 - Bay Area Transit Frequency')
map = Basemap(resolution="h", projection="stere", width=80000, height=80000, lon_0=-122.2913, lat_0=37.8272)

plotData(map, ['WestCAT Data'], min_draw_size)

plotData(map, ['SF Ferries Data'], min_draw_size)

plotData(map, ['Marin Transit Data'], min_draw_size)

plotData(map, ['Golden Gate Transit Data'], min_draw_size)

# Missing shapes.txt.
# plotData(map, ['County Connection Data'], min_draw_size)

plotData(map, ['SamTrans Data'], min_draw_size)

plotData(map, ['AC Transit Data'], min_draw_size)

plotData(map, ['VTA Data'], min_draw_size)

plotData(map, ['Muni Data'], min_draw_size)

plotData(map, ['BART Data'], min_draw_size * 2)

bart_stops = pd.read_csv('../BART Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
for index, row in bart_stops.iterrows():
...
# Could not read Caltrain data because python doesn't like quotes.
# plotData(map, ['Caltrain Data'], min_draw_size * 2)

# caltrain_stops = pd.read_csv('../Caltrain Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
# for index, row in caltrain_stops.iterrows():
    # map.plot(row['stop_lon'], row['stop_lat'], marker='o', markersize=min_draw_size, markeredgecolor='white', markerfacecolor='black', markeredgewidth=min_draw_size * .5, latlon=True)
