...
plt.title('7 - Philadelphia Transit Frequency')
map = Basemap(resolution="h", projection="stere", width=80000, height=80000, lon_0=-75.1637900, lat_0=39.9523400)

plotData(map, ['DART Data'], min_draw_size)

plotData(map, ['SEPTA Bus Data'], min_draw_size)

# NJT's bus data is missing calendar.txt. Isn't this a required file for GTFS?
# plotData(map, ['NJT Bus Data'], min_draw_size * 2)

plotData(map, ['SEPTA Rail Data'], min_draw_size * 2)

septa_rail_stops = pd.read_csv('../SEPTA Rail Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
for index, row in septa_rail_stops.iterrows():
...
plotData(map, ['PATCO Data'], min_draw_size * 2)

patco_stops = pd.read_csv('../PATCO Data/stops.txt')[stopData]

# Make cute little stop circles for the stops.
for index, row in patco_stops.iterrows():
    map.plot(row['stop_lon'], row['stop_lat'], marker='o', markersize=min_draw_size, markeredgecolor='white', markerfacecolor='black', markeredgewidth=min_draw_size * .5, latlon=True)
